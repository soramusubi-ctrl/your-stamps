(() => {
  ...
  const getImageSize = (hasTwoLines) => {
    const base = hasTwoLines ? 188 : 210;
    if (layout.imageSize === 'small') return base - 28;
    if (layout.imageSize === 'large') return base + 24;
    return base;
  };

  const getTextStartY = (lineCount) => {
    const two = lineCount >= 2;
    const map = {
      top: two ? 170 : 188,
      middle: two ? 194 : 214,
      bottom: two ? 214 : 236,
    };
    return map[layout.textY] || map.bottom;
  };
  ...
  const composeSticker = async (imageSrc, text) => {
    ...
    const imageSize = getImageSize(hasTwoLines);
    const scale = Math.min(imageSize / img.width, imageSize / img.height);
    const drawW = img.width * scale;
    const drawH = img.height * scale;
    const x = (canvas.width - drawW) / 2;
    const y = layout.textY === 'top' ? 50 : hasTwoLines ? 2 : 6;
    ...
    if (lines.length > 0) {
      ...
      const startY = getTextStartY(lines.length);
  ...
  const saveToGallery = (dataUrlList) => {
    ...
    const newItems = dataUrlList.map((data, index) => ({ id: `oneshot-${Date.now()}-${index}`, data, date: now }));
  ...
  const generateImages = async (prompt, expressions) => {
    const response = await fetch('/api/generate', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ prompt, count: expressions.length, customExpressions: expressions }),
    });
    if (!response.ok) {
      const data = await response.json().catch(() => ({}));
  ...
  const applySegmentState = (panel, attr, value) => {
    panel.querySelectorAll(`[${attr}]`).forEach((button) => {
  ...
  const setMode = (panel, mode) => {
    ...
    applySegmentState(panel, 'data-mode-button', mode);
  ...
  const setLayout = (panel, key, value) => {
    layout[key] = value;
    applySegmentState(panel, key === 'textY' ? 'data-text-y' : 'data-image-size', value);
  };
  ...
  const makeButtonStyle = (active = false) => `padding:10px;border-radius:14px;border:1px solid ${active ? '#16a34a' : '#e2e8f0'};background:${active ? '#16a34a' : '#fff'};color:${active ? '#fff' : '#475569'};font-weight:900;text-align:center;`;
  ...
  const injectPanel = () => {
    ...
    panel.innerHTML = `
      <details>
        <summary style="cursor:pointer;display:flex;align-items:center;justify-content:space-between;gap:12px;list-style:none;">
          <div><div style="font-size:18px;font-weight:1000;color:#0f172a;letter-spacing:-.03em;">申請セットを一気に作る</div><div style="font-size:12px;color:#64748b;line-height:1.5;">8個生成・文字入れ・透過・申請文案まで</div></div>
        ...
        <div style="padding-top:12px;">
          <div style="font-size:11px;font-weight:900;color:#475569;margin-bottom:6px;">作り方</div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px;">
            <button data-mode-button="ai" type="button" style="${makeButtonStyle(true)}">AIで8個<br><span style="font-size:10px;font-weight:600;opacity:.85;">画像も生成</span></button>
            <button data-mode-button="direct" type="button" style="${makeButtonStyle(false)}">画像そのまま<br><span style="font-size:10px;font-weight:600;opacity:.85;">文字だけ変更</span></button>
          </div>
          ...
          <div data-file-box style="display:none;margin-top:10px;"><label style="display:block;padding:12px;border-radius:16px;border:1px dashed #bbf7d0;background:#f0fdf4;color:#047857;font-weight:900;text-align:center;font-size:13px;cursor:pointer;">手描き画像・子どもの絵を選ぶ<input data-oneshot-file type="file" accept="image/*" style="display:none;" /></label><div data-oneshot-file-name style="font-size:11px;color:#64748b;margin-top:6px;"></div></div>
          <div style="font-size:11px;font-weight:900;color:#475569;margin:12px 0 6px;">文字位置</div>
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;">
            <button data-text-y="top" type="button" style="${makeButtonStyle(false)}">上</button>
            <button data-text-y="middle" type="button" style="${makeButtonStyle(false)}">中</button>
            <button data-text-y="bottom" type="button" style="${makeButtonStyle(true)}">下</button>
          </div>
          <div style="font-size:11px;font-weight:900;color:#475569;margin:12px 0 6px;">画像サイズ</div>
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;">
            <button data-image-size="small" type="button" style="${makeButtonStyle(false)}">小</button>
            <button data-image-size="normal" type="button" style="${makeButtonStyle(true)}">標準</button>
            <button data-image-size="large" type="button" style="${makeButtonStyle(false)}">大</button>
          </div>
          <button data-oneshot-run type="button" style="width:100%;margin-top:12px;padding:13px;border-radius:16px;background:#16a34a;color:white;font-weight:1000;border:none;font-size:14px;">作成する</button>
    ...
    panel.querySelectorAll('[data-mode-button]').forEach((button) => button.addEventListener('click', () => setMode(panel, button.dataset.modeButton)));
    panel.querySelectorAll('[data-text-y]').forEach((button) => button.addEventListener('click', () => setLayout(panel, 'textY', button.dataset.textY)));
    panel.querySelectorAll('[data-image-size]').forEach((button) => button.addEventListener('click', () => setLayout(panel, 'imageSize', button.dataset.imageSize)));
